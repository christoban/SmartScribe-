...
async def export_note(
    ...
    export_id = await ExportRepository.create(export_data.model_dump())
    
    logger.info("📄 Export %s créé pour note %s", format, note_id)
    
    return await ExportRepository.get_by_id(export_id)
...
async def download_export(
    ...
    export = await ExportRepository.get_by_id(export_id)
...
async def list_exports(
    ...
    if note_id:
        return await ExportRepository.get_by_note_id(note_id, str(current_user.id))
    return await ExportRepository.get_user_exports(str(current_user.id))
