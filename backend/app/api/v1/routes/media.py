...
async def upload_media(
    ...
    media_id = await MediaRepository.create(media_obj)
    ...
    return await MediaRepository.get_by_id(media_id)

@router.get("/", response_model=List[MediaOut])
async def list_my_media(current_user = Depends(get_current_user)):
    return await MediaRepository.get_user_media(str(current_user.id))
